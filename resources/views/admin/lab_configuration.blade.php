...
<body class="flex h-screen w-full bg-slate-50 dark:bg-[#020617] text-slate-800 dark:text-slate-200 transition-colors duration-500" x-data="{ 
        ...
        <aside class="glass-sidebar w-72 h-full flex flex-col fixed md:relative z-[100] transition-transform duration-300 transform md:translate-x-0" :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'">
            <div class="h-24 flex items-center justify-between px-8 border-b border-slate-200 dark:border-white/5 relative overflow-hidden group transition-colors">
                ...
                <a href="{{ route('landing') ?? '#' }}" class="flex items-center gap-3 relative z-10">
                    <img src="{{ asset('images/logo.png') }}" onerror="this.src='https://via.placeholder.com/40'" class="h-8 w-auto object-contain block dark:hidden" style="filter: brightness(0.1);" alt="Logo">
                    <img src="{{ asset('images/logo.png') }}" onerror="this.src='https://via.placeholder.com/40'" class="h-8 w-auto object-contain hidden dark:block drop-shadow-sm" alt="Logo Dark">
                    <div>
                ...
                <div>
                    ...
                    <div class="space-y-1">
                        ...
                        <a href="{{ route('admin.lab.analytics') ?? '#' }}" class="nav-link">
                            <svg class="w-5 h-5 text-slate-400 dark:text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 12l3-3 3 3 4-4M8 21l4-4 4 4M3 4h18M4 4h16v12a1 1 0 01-1 1H5a1 1 0 01-1-1V4z"/></svg>
                            Lab Analytics
                        </a>
                        <a href="{{ route('admin.classes.index') ?? '#' }}" class="nav-link">
                            <svg class="w-5 h-5 text-slate-400 dark:text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                            Class Management
                        </a>
                    </div>
                </div>
            ...
            <div class="p-4 border-t border-slate-200 dark:border-white/5 bg-slate-50/50 dark:bg-[#05080f]/50 transition-colors">
                <div class="flex items-center gap-3 mb-4 px-2">
                    <div class="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center font-bold text-white text-xs border border-white/10 shadow-md">AD</div>
                ...
                <button id="theme-toggle-sidebar" type="button" class="w-full mb-2 flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-slate-200/50 dark:bg-white/5 hover:bg-slate-300/50 dark:hover:bg-white/10 text-slate-700 dark:text-slate-300 transition-colors border border-transparent dark:border-transparent text-xs font-bold shadow-sm dark:shadow-none">
            ...
            <div class="flex-1 p-4 md:p-8 lg:p-10 relative z-10">
                <div class="max-w-7xl mx-auto space-y-8">
                    ...
                    <div class="glass-card rounded-2xl flex flex-col transition-colors duration-300 z-10 reveal shadow-sm dark:shadow-[0_4px_30px_rgba(0,0,0,0.2)]" style="animation-delay: 0.1s;">
                        ...
                        {{-- Table --}}
                        <div class="overflow-x-auto custom-scrollbar relative p-0 sm:p-6 pt-0 border-t border-slate-200 dark:border-white/5 sm:border-none transition-colors">
                            <table class="w-full text-sm text-left border-collapse min-w-[700px] border border-slate-200 dark:border-white/5 sm:rounded-xl shadow-sm dark:shadow-inner bg-white/50 dark:bg-[#0a0e17]/30 transition-colors">
                                ...
                                <tbody class="divide-y divide-slate-200 dark:divide-white/5 relative z-10 transition-colors" id="labTableBody">
                                    @forelse($labs ?? [] as $lab)
                                    <tr class="table-row group hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
                                        <td class="px-6 py-4">
                                            <div class="flex items-center gap-4">
                                                <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center font-bold text-white text-sm shadow-md dark:shadow-inner border border-transparent dark:border-white/10 group-hover:shadow-[0_0_15px_rgba(99,102,241,0.5)] transition shrink-0">
                                                    {{ substr($lab->title, 0, 1) }}
                                                </div>
                                                <div class="min-w-0">
                                                    <p class="font-bold text-slate-900 dark:text-white text-xs group-hover:text-indigo-600 dark:group-hover:text-indigo-300 transition-colors truncate">{{ $lab->title }}</p>
                                                    <p class="text-[9px] text-slate-500 dark:text-white/40 mt-0.5 line-clamp-1 transition-colors">{{ $lab->description }}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4">
                                            <span class="font-mono text-[10px] text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-[#020617] border border-indigo-200 dark:border-white/10 px-3 py-1.5 rounded-lg shadow-sm dark:shadow-inner transition-colors">{{ $lab->slug }}</span>
                                        </td>
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            <span class="px-3 py-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20 text-[9px] font-bold uppercase tracking-widest shadow-sm dark:shadow-none transition-colors">Min {{ $lab->passing_grade }}</span>
                                        </td>
    ...
    <script>
        ...
        // --- GLOBAL SWAL BACKGROUND ADAPTER ---
        function getSwalTheme() {
            ...
            return {
                ...
                borderClass: 'rounded-2xl border border-slate-200 dark:border-white/10 shadow-xl dark:shadow-[0_10px_50px_rgba(0,0,0,0.8)]'
            };
        }
