...
@section('content')
<div class="min-h-screen bg-slate-50 dark:bg-[#020617] text-slate-800 dark:text-white font-sans selection:bg-fuchsia-500/30 pb-20 pt-28 md:pt-32 transition-colors duration-500">

    {{-- ==================== BACKGROUND FX ==================== --}}
    <div class="fixed inset-0 -z-50 pointer-events-none overflow-hidden">
        <div class="absolute top-[-10%] left-[-10%] w-[500px] h-[500px] bg-fuchsia-300/30 dark:bg-fuchsia-600/10 rounded-full blur-[120px] animate-pulse transition-colors duration-500"></div>
    ...
    <main class="max-w-7xl mx-auto px-5 lg:px-8 relative z-10">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8 md:mb-12 reveal-up">
            <div>
                ...
                <nav class="flex items-center gap-2 mb-3 text-[10px] md:text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-white/40 transition-colors" aria-label="Breadcrumb">
                    ...
                    <span class="text-slate-300 dark:text-white/20 transition-colors">/</span>
                    <span class="text-fuchsia-600 dark:text-fuchsia-400 drop-shadow-none dark:drop-shadow-[0_0_8px_rgba(217,70,239,0.5)] transition-colors">Profil Saya</span>
                </nav>

                <h1 class="text-3xl md:text-5xl font-black text-slate-900 dark:text-white tracking-tight mb-2 transition-colors">
                    Profil <span class="text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-600 to-cyan-600 dark:from-fuchsia-400 dark:to-cyan-400">Pengguna</span>
                </h1>
                <p class="text-slate-500 dark:text-white/50 text-sm md:text-base transition-colors">Kelola informasi pribadi, identitas akademik, dan lihat jejak karir Anda.</p>
            </div>
        </div>
        ...
        <div class="grid lg:grid-cols-12 gap-6 md:gap-8 items-start reveal-up delay-100">
            ...
            <div class="lg:col-span-4 space-y-6">
                ...
                <div class="relative bg-white/80 dark:bg-[#0f141e]/80 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-[2rem] p-6 md:p-8 text-center overflow-hidden shadow-xl dark:shadow-2xl group/profile transition-colors duration-500">
                    ...
                    <div class="absolute -top-24 -right-24 w-48 h-48 bg-cyan-400/20 dark:bg-cyan-500/10 rounded-full blur-[50px] group-hover/profile:bg-cyan-400/30 dark:group-hover/profile:bg-cyan-500/20 transition-all duration-700"></div>
                    
                    {{-- Avatar Wrapper --}}
                    <div class="relative w-32 h-32 mx-auto mb-6">
                        <div class="absolute inset-0 bg-gradient-to-tr from-fuchsia-400 to-cyan-400 dark:from-fuchsia-500 dark:to-cyan-500 rounded-full blur-md opacity-40 group-hover/profile:opacity-70 transition duration-500 animate-spin-slow"></div>
                        
                        @if(Auth::user()->avatar)
                            <img src="{{ asset('storage/' . Auth::user()->avatar) }}" 
                                 class="relative w-full h-full object-cover rounded-full border-4 border-white dark:border-[#0f141e] shadow-md dark:shadow-xl transition-all duration-500 hover:scale-105 cursor-pointer" 
                                 alt="Profile Photo" id="avatar-display">
                        @else
                            <div class="relative w-full h-full flex items-center justify-center bg-gradient-to-br from-slate-200 to-slate-400 dark:from-slate-700 dark:to-slate-900 rounded-full border-4 border-white dark:border-[#0f141e] text-4xl font-black text-slate-700 dark:text-white shadow-md dark:shadow-xl transition-all duration-500 hover:scale-105 cursor-pointer" id="avatar-display">
                        ...
                        <div class="absolute bottom-0 right-0 translate-x-2 translate-y-1 z-10">
                            <span class="flex items-center gap-1.5 px-3 py-1 bg-white dark:bg-[#020617] border border-slate-200 dark:border-white/10 rounded-full text-[10px] font-bold uppercase tracking-widest text-cyan-600 dark:text-cyan-400 shadow-sm dark:shadow-lg transition-colors">
                    ...
                    <p class="text-xs text-slate-500 dark:text-white/40 font-mono mt-1 mb-8 truncate transition-colors">{{ Auth::user()->email }}</p>

                    {{-- GAMIFICATION: Rank & Progress --}}
                    <div class="bg-slate-50 dark:bg-[#020617] rounded-2xl p-5 border border-slate-200 dark:border-white/5 shadow-inner text-left relative overflow-hidden transition-colors duration-500">
                        <div class="absolute top-0 right-0 w-32 h-32 bg-fuchsia-400/10 dark:bg-fuchsia-500/5 rounded-full blur-[30px] pointer-events-none transition-colors"></div>
                        
                        <div class="flex justify-between items-end mb-2 relative z-10">
                            <div>
                                <p class="text-[9px] uppercase tracking-widest text-slate-500 dark:text-white/40 font-bold mb-1 transition-colors">Developer Rank</p>
                                <h3 class="text-lg font-black text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-600 to-cyan-600 dark:from-fuchsia-400 dark:to-cyan-400 transition-colors">
                                    {{ Auth::user()->developer_title ?? 'CSS Novice' }}
                                </h3>
                            </div>
                            <div class="text-right">
                                <span class="text-xl font-black text-slate-900 dark:text-white transition-colors">{{ number_format(Auth::user()->xp ?? ($stats['xp'] ?? 0)) }}</span>
                                <span class="text-[10px] text-fuchsia-600 dark:text-fuchsia-400 font-bold ml-1 transition-colors">XP</span>
                            </div>
                        </div>

                        {{-- Progress Bar Container --}}
                        <div class="w-full h-2.5 bg-slate-200 dark:bg-slate-800 rounded-full overflow-hidden mt-4 relative z-10 border border-slate-300 dark:border-white/5 transition-colors">
                            @php
                        ...
                        <div class="flex justify-between items-center mt-2 relative z-10">
                            <span class="text-[9px] text-slate-500 font-mono font-bold transition-colors">{{ $xpPercent }}% Selesai</span>
                            <span class="text-[9px] text-slate-500 dark:text-slate-400 font-bold transition-colors">Target: {{ number_format(Auth::user()->next_level_xp ?? 300) }} XP</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    {{-- Avg Score --}}
                    <div class="p-5 rounded-[2rem] bg-white/80 dark:bg-[#0f141e]/80 backdrop-blur-md border border-slate-200 dark:border-white/10 shadow-md dark:shadow-xl flex flex-col justify-center items-center text-center group hover:border-cyan-400 dark:hover:border-cyan-500/30 transition-colors duration-500">
                        <div class="w-10 h-10 bg-cyan-50 dark:bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 rounded-2xl flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2-2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
                        </div>
                        <p class="text-3xl font-black text-slate-900 dark:text-white transition-colors">{{ $stats['avg_score'] ?? 0 }}</p>
                        <p class="text-[9px] text-slate-500 dark:text-white/40 uppercase font-bold tracking-widest mt-1 transition-colors">Rata-rata Kuis</p>
                    </div>

                    {{-- Completed Labs --}}
                    <div class="p-5 rounded-[2rem] bg-white/80 dark:bg-[#0f141e]/80 backdrop-blur-md border border-slate-200 dark:border-white/10 shadow-md dark:shadow-xl flex flex-col justify-center items-center text-center group hover:border-indigo-400 dark:hover:border-indigo-500/30 transition-colors duration-500">
                        <div class="w-10 h-10 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 rounded-2xl flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                        ...
                        <p class="text-3xl font-black text-slate-900 dark:text-white transition-colors">{{ $stats['labs_done'] ?? 0 }}</p>
                        <p class="text-[9px] text-slate-500 dark:text-white/40 uppercase font-bold tracking-widest mt-1 transition-colors">Lab Diselesaikan</p>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-white/90 dark:bg-[#0f141e]/90 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-[2rem] p-6 md:p-10 shadow-xl dark:shadow-2xl relative overflow-hidden transition-colors duration-500">
                    <div class="flex items-center gap-4 mb-8 border-b border-slate-200 dark:border-white/5 pb-6 relative z-10 transition-colors">
                        ...
                        <div>
                            ...
                            <p class="text-xs md:text-sm text-slate-500 dark:text-white/40 mt-1 transition-colors">Perbarui foto profil dan data identitas Anda.</p>
                        </div>
                    </div>
                        ...
                        <div class="flex flex-col sm:flex-row items-center gap-6 p-5 rounded-2xl bg-slate-50 dark:bg-[#020617] border border-slate-200 dark:border-white/10 shadow-sm dark:shadow-inner transition-colors">
                            <div class="shrink-0 relative group cursor-pointer" id="avatar-trigger">
                                <img id="preview-img" 
                                     ...
                                     class="h-20 w-20 object-cover rounded-full border-2 border-slate-200 dark:border-white/10 shadow-md dark:shadow-lg transition-all duration-300 group-hover:border-cyan-400 dark:group-hover:border-cyan-500/50" />
