...
<nav id="navbar" class="fixed top-0 left-0 right-0 z-[100] w-full h-[76px] md:h-[88px] bg-white/90 dark:bg-[#020617]/90 backdrop-blur-2xl border-b border-slate-200/80 dark:border-white/[0.05] transition-all duration-500">
    ...
    <div class="w-full h-full px-5 md:px-10 lg:px-16 max-w-[1440px] mx-auto">
        <div class="flex items-center justify-between h-full">
            ...
            {{-- 1. KIRI: LOGO BRAND (Ukuran Besar & Proporsional) --}}
            ...
            <div class="hidden md:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 items-center gap-8 z-20">
                @foreach([
                    ...
                    'Materi' => route('courses.curriculum'), 
                    'Sandbox' => route('sandbox')
                ] as $label => $link)
                    <a href="{{ $link }}" class="text-[14px] md:text-[15px] font-semibold text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-cyan-400 transition-colors duration-300 tracking-wide focus:outline-none">
                        {{ $label }}
                    </a>
                @endforeach
                ...
                <div class="relative" id="resource-menu-container">
                    <button onclick="toggleResourceMenu()" class="flex items-center gap-1.5 text-[14px] md:text-[15px] font-semibold text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-cyan-400 transition-colors duration-300 tracking-wide focus:outline-none group">
                        Resources
                        <svg id="resource-chevron" class="w-4 h-4 text-slate-400 group-hover:text-indigo-500 dark:group-hover:text-cyan-400 transition-transform duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
                    </button>
                    ...
                    <div id="resource-dropdown" class="hidden absolute top-[calc(100%+18px)] left-1/2 -translate-x-1/2 w-[260px] bg-white dark:bg-[#0a0e17] border border-slate-200/80 dark:border-white/10 rounded-2xl shadow-xl dark:shadow-[0_16px_40px_-8px_rgba(0,0,0,0.8)] overflow-hidden transform transition-all duration-200 opacity-0 scale-95 origin-top p-1.5 z-50">
                        <a href="{{ route('cheatsheet.index') }}" class="flex items-start gap-3.5 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-white/5 transition-colors focus:outline-none group">
                            ...
                            <div>
                                <span class="block text-[14px] font-bold text-slate-900 dark:text-white leading-none mb-1.5 group-hover:text-fuchsia-600 dark:group-hover:text-fuchsia-400 transition-colors">Cheat Sheet</span>
                        ...
                        <a href="{{ route('gallery.index') }}" class="flex items-start gap-3.5 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-white/5 transition-colors focus:outline-none group mt-1">
                            ...
                            <div>
                                <span class="block text-[14px] font-bold text-slate-900 dark:text-white leading-none mb-1.5 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors">UI Gallery</span>
                        ...
                        <div class="relative" id="desktop-user-menu">
                            <button onclick="toggleUserMenu()" class="flex items-center gap-3 py-1.5 pl-1.5 pr-3.5 rounded-full hover:bg-slate-100 dark:hover:bg-white/10 transition-colors focus:outline-none group">
                                <div class="relative">
                                    ...
                                    @if(Auth::user()->avatar)
                                        @php
                                            $avatarPath = \Illuminate\Support\Str::startsWith(Auth::user()->avatar, ['http://', 'https://']) 
                                                ? Auth::user()->avatar 
                                        ...
                                        <img src="{{ $avatarPath }}" 
                                             ...
                                             class="w-9 h-9 rounded-full object-cover border border-slate-200 dark:border-white/10 shadow-sm">
                                    @else
                                        <div class="w-9 h-9 rounded-full bg-slate-900 dark:bg-white flex items-center justify-center text-xs font-bold text-white dark:text-slate-900 shadow-sm">
                                ...
                                <svg id="menu-chevron" class="w-4 h-4 text-slate-400 group-hover:text-indigo-500 transition-transform duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M19 9l-7 7-7-7" /></svg>
                            </button>
                            ...
                            <div id="user-dropdown" class="hidden absolute right-0 top-[calc(100%+16px)] w-64 bg-white dark:bg-[#0f141e] border border-slate-200/80 dark:border-white/10 rounded-2xl shadow-xl dark:shadow-[0_16px_40px_-8px_rgba(0,0,0,0.8)] overflow-hidden transform transition-all duration-200 opacity-0 scale-95 origin-top-right z-50">
                                ...
                                <div class="p-2 space-y-1">
                                    @if(auth()->user()->role === 'admin')
                                        <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-semibold text-slate-700 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 transition-colors">
    ...
    <div id="mobile-menu" class="hidden md:hidden fixed inset-0 top-[76px] bg-white dark:bg-[#020617] transition-all duration-300 opacity-0 transform translate-y-4 h-[calc(100dvh-76px)] overflow-y-auto z-[90] border-t border-slate-200 dark:border-white/5">
        ...
        <div class="flex flex-col min-h-full px-5 py-8">
            ...
            @auth
                {{-- KARTU PROFIL MOBILE YANG DAPAT DIKLIK (AKSES KE PENGATURAN) --}}
                <a href="{{ route('profile.edit') }}" class="flex items-center gap-5 mb-8 p-5 rounded-[1.5rem] border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#0a0e17] shadow-sm hover:border-indigo-300 dark:hover:border-cyan-500/30 transition-all group focus:outline-none">
                    @if(Auth::user()->avatar)
                        @php
                            $avatarPathMob = \Illuminate\Support\Str::startsWith(Auth::user()->avatar, ['http://', 'https://']) 
                                ? Auth::user()->avatar 
                                : asset('uploads/' . Auth::user()->avatar);
                        @endphp
                        <img src="{{ $avatarPathMob }}" 
                             onerror="this.onerror=null;this.src='https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name) }}&background=6366f1&color=fff';"
                             class="w-16 h-16 rounded-full object-cover border border-slate-200 dark:border-white/10 group-hover:scale-105 transition-transform duration-300">
                    @else
                        <div class="w-16 h-16 rounded-full bg-slate-900 dark:bg-white flex items-center justify-center font-bold text-white dark:text-slate-900 text-xl border border-transparent group-hover:scale-105 transition-transform duration-300">
                            {{ substr(Auth::user()->name, 0, 2) }}
                        </div>
                    @endif
                    <div class="flex-1 min-w-0">
                        <p class="text-slate-900 dark:text-white font-black text-xl truncate group-hover:text-indigo-600 dark:group-hover:text-cyan-400 transition-colors">{{ Auth::user()->name }}</p>
                        <p class="text-[13px] text-slate-500 dark:text-slate-400 truncate mt-0.5">Lihat & edit profil</p>
                    </div>
                    <div class="w-8 h-8 rounded-full bg-white dark:bg-[#1d1d1f] border border-slate-200 dark:border-white/10 flex items-center justify-center text-slate-400 group-hover:text-indigo-600 dark:group-hover:text-cyan-400 transition-colors shadow-sm shrink-0">
            ...
            <div class="mt-10 pt-6 border-t border-slate-200 dark:border-white/10 pb-6">
                @auth
                    <div class="flex flex-col gap-3">
                        ...
                        <a href="{{ route('profile.edit') }}" class="w-full py-4 rounded-xl border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 font-bold text-[16px] text-center hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">Pengaturan Akun</a>
