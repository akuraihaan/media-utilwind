{{-- NAVBAR COMPONENT (TALL & ELEGANT SAAS EDITION - HOSTING FIXED) --}}
    ...
    <div class="w-full h-full px-5 md:px-10 lg:px-16 max-w-[1440px] mx-auto">
        <div class="flex items-center justify-between h-full">
            ...
            <div class="hidden md:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 items-center gap-8 z-20">
                ...
                <div class="relative" id="resource-menu-container">
                    ...
                    <div id="resource-dropdown" class="hidden absolute top-[calc(100%+18px)] left-1/2 -translate-x-1/2 w-[260px] bg-white dark:bg-[#0a0e17] border border-slate-200/80 dark:border-white/10 rounded-2xl shadow-xl dark:shadow-[0_16px_40px_-8px_rgba(0,0,0,0.8)] overflow-hidden transform transition-all duration-200 opacity-0 scale-95 origin-top p-1.5 z-50">
                        <a href="{{ route('cheatsheet.index') }}" class="flex items-start gap-3.5 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group">
                        ...
                        <a href="{{ route('gallery.index') }}" class="flex items-start gap-3.5 p-3 rounded-xl hover:bg-slate-50 dark:hover:bg-white/5 transition-colors group mt-1">
            ...
            <div class="flex items-center gap-2 md:gap-4 shrink-0 z-30">
                <button class="theme-toggle-btn flex items-center justify-center w-10 h-10 rounded-full text-slate-400 hover:text-slate-800 dark:text-slate-500 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/10 transition-all focus:outline-none">
                ...
                <div class="hidden md:flex items-center">
                    @auth
                        <div class="relative" id="desktop-user-menu">
                            <button onclick="toggleUserMenu()" class="flex items-center gap-3 py-1.5 pl-1.5 pr-3.5 rounded-full hover:bg-slate-100 dark:hover:bg-white/10 transition-colors focus:outline-none group">
                                <div class="relative">
                                    @if(Auth::user()->avatar)
                                        {{-- FIXED PATH: Menggunakan uploads/ bukan storage/ --}}
                                        <img src="{{ asset('uploads/' . Auth::user()->avatar) }}" class="w-9 h-9 rounded-full object-cover border border-slate-200 dark:border-white/10">
                                    @else
                    ...
                    @else
                        <a href="{{ route('login') }}" class="px-4 py-2 text-[14px] font-semibold text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white transition-colors">Masuk</a>
                        <a href="{{ route('register') }}" class="ml-2 h-[42px] px-6 flex items-center justify-center bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-[14px] font-bold rounded-full hover:scale-105 transition-transform duration-300 shadow-md">Daftar</a>
                    @endauth
                </div>

                {{-- MOBILE HAMBURGER --}}
                <div class="md:hidden">
                    <button onclick="toggleMobileMenu()" class="flex items-center justify-center w-11 h-11 rounded-full text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-white/10 transition-colors relative z-[110]">
                        <svg id="hamburger-icon" class="w-6 h-6 absolute" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 6h16M4 12h16M4 18h16" /></svg>
                        <svg id="close-icon" class="w-6 h-6 absolute opacity-0 scale-50" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                </div>
        ...
        <div class="flex flex-col min-h-full px-5 py-8">
            @auth
                <div class="flex items-center gap-5 mb-8 p-5 rounded-[1.5rem] border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#0a0e17] shadow-sm">
                    @if(Auth::user()->avatar)
                        {{-- FIXED PATH MOBILE --}}
                        <img src="{{ asset('uploads/' . Auth::user()->avatar) }}" class="w-16 h-16 rounded-full object-cover border border-slate-200 dark:border-white/10">
                    @else
            ...
            <div class="flex flex-col gap-2 flex-1">
                @foreach([['label' => 'Beranda', 'link' => route('landing')], ['label' => 'Materi Belajar', 'link' => route('courses.curriculum')], ['label' => 'Live Sandbox', 'link' => route('sandbox')]] as $menu)
                    <a href="{{ $menu['link'] }}" class="flex items-center justify-between p-4 rounded-2xl text-slate-800 dark:text-slate-200 font-bold text-[18px] hover:bg-slate-50 dark:hover:bg-white/5 transition-colors">
            ...
            <div class="mt-10 pt-6 border-t border-slate-200 dark:border-white/10 pb-6">
                @auth
                    <a href="{{ route('dashboard') }}" class="w-full py-4 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold text-center shadow-lg block">Buka Dasbor Belajar</a>
                @else
                    <div class="flex flex-col gap-3">
                        <a href="{{ route('register') }}" class="w-full py-4 rounded-xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-center font-bold shadow-lg block">Daftar Sekarang</a>
                        <a href="{{ route('login') }}" class="w-full py-4 rounded-xl border border-slate-300 dark:border-white/20 text-slate-700 dark:text-white text-center font-bold block">Masuk</a>
                    </div>
                @endauth
            </div>
        </div>
...
<script>
    function toggleUserMenu() {
        ...
        if (menu.classList.contains('hidden')) {
            ...
            setTimeout(() => { menu.classList.remove('opacity-0', 'scale-95'); chevron.classList.add('rotate-180'); }, 10);
        } else {
            menu.classList.add('opacity-0', 'scale-95'); chevron.classList.remove('rotate-180');
    ...
    document.addEventListener('click', function(e) {
        if (!document.getElementById('desktop-user-menu')?.contains(e.target)) {
            const menu = document.getElementById('user-dropdown');
            if (menu && !menu.classList.contains('hidden')) toggleUserMenu();
        }
        if (!document.getElementById('resource-menu-container')?.contains(e.target)) {
            const menu = document.getElementById('resource-dropdown');
            if (menu && !menu.classList.contains('hidden')) toggleResourceMenu();
        }
    });
</script>