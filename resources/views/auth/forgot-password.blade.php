...
@section('title','Lupa Password · Flowwind Learn')
@section('heading','Reset Password')
@section('subheading','Link reset akan dikirim ke email Anda')
    ...
    <div class="w-full max-w-[450px] relative z-20 animate-fade-in-up">
        ...
        <div class="relative bg-[#0f141e]/70 backdrop-blur-xl border border-white/10 rounded-3xl p-8 md:p-10 shadow-2xl shadow-black/50 overflow-hidden group">
            ...
            <div class="text-center mb-10">
                ...
                <h1 class="text-2xl font-black text-white mb-2">@yield('heading')</h1>
                <p class="text-white/50 text-sm">@yield('subheading')</p>
            </div>

            @if (session('status'))
                <div class="mb-6 rounded-xl bg-emerald-500/10 border border-emerald-500/20 p-4 text-sm font-medium text-emerald-400 flex items-center gap-3 shadow-[0_0_15px_rgba(16,185,129,0.1)]">
            ...
            <form method="POST" action="{{ route('password.email') }}" class="space-y-6">
                ...
                <div class="text-center">
                    <a href="{{ route('login') }}" class="group inline-flex items-center gap-2 text-sm text-cyan-400 font-bold hover:text-cyan-300 transition-colors">
